...
import sigsJSON from "./sigs_batch.json";

function App() {
  const worker = new Worker(new URL("./nova-scotia-worker", import.meta.url), {
    name: "nova-scotia-worker-ecdsa",
    type: "module",
  });
  const workerApi =
    wrap<import("./nova-scotia-worker").NovaScotiaWorkerECDSA>(worker);
  ...
  const sigs = JSON.stringify(sigsJSON);
  ...
  async function generate_proof() {
    ...
    const proof = await workerApi.generate_proof(pp, sigs);
  ...
  async function verify_proof() {
    ...
    const res = await workerApi.verify_proof(pp, proof, sigs);
